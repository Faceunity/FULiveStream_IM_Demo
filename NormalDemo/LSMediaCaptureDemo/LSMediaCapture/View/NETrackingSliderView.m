...
//        self.alpha = 0.5;
        ...
        self.volumeLabel = [[UILabel alloc] initWithFrame:CGRectMake(20, 120, 100, 40)];
        self.volumeLabel.text = @"混音强度:";
        self.volumeLabel.textColor = [UIColor blackColor];
        self.volumeLabel.textAlignment = NSTextAlignmentLeft;
...
//        self.slider.maximumValue = _mediaCapture.maxZoomScale;
        ...
        self.voulmeSlider = [[ASValueTrackingSlider alloc] initWithFrame:CGRectMake(100, 120, 200, 40)];
        self.voulmeSlider.minimumValue = 1;
        self.voulmeSlider.maximumValue = 10;
        self.voulmeSlider.popUpViewCornerRadius = 0.0;
        [self.voulmeSlider setMaxFractionDigitsDisplayed:0];
        self.voulmeSlider.popUpViewColor = [UIColor colorWithHue:0.55 saturation:0.8 brightness:0.9 alpha:0.7];
        self.voulmeSlider.textColor = [UIColor colorWithHue:0.55 saturation:1.0 brightness:0.5 alpha:1];
        [self.voulmeSlider setValue:10.0];
        self.voulmeSlider.tag = 5;
        //滑动拖动后的事件
        [self.voulmeSlider addTarget:self action:@selector(sliderValueChanged:) forControlEvents:UIControlEventValueChanged];
        //如果打开拖拽
        if (paraCtx.isCameraZoomPinchGestureOn) {
            [self addSubview:self.voulmeSlider];
        }
    }
    return self;
}
